...
* The high R-squared gives false confidence. The model appears perfect, but it’s estimating the wrong relationship because it’s missing a key variable.
...
with $R^2 = 0.563$. Under this specification, the intercept suggests −3.68 anxiety units at zero social media use, which is conceptually implausible, and the slope coefficient of 5.341 indicates that each additional hour of social media use is associated with a 5.34-unit increase in anxiety. 
...
This analysis demonstrates the critical importance of measurement quality and functional form assumptions in linear regression models. Through systematic examination of bivariate and multiple regression specifications using both proxy and true stress measurements, several key findings emerge.

First, strong statistical fit measures (e.g., high $R^2$ values) and statistically significant coefficients do not guarantee accurate parameter estimates. The proxy-based multiple regression model achieved an $R^2$ of 0.935 and statistically significant coefficients, yet produced substantially biased estimates, including a coefficient for social media use that reversed sign relative to the true causal effect.

Second, the use of proxy variables with non-linear relationships to true causal variables can introduce systematic bias even when the proxy maintains a monotonic relationship. The bivariate regression using StressSurvey produced a slope coefficient of 1.047 (compared to the true value of 1.0), and when included in a multiple regression, the proxy's non-linearity caused the Time coefficient to flip from its true positive value of +0.100 to a negative value of -2.780.

Third, omitted-variable bias can dramatically inflate coefficient estimates when important confounding variables are excluded. The bivariate regression of Anxiety on Time alone produced a coefficient estimate of 5.341, approximately 53 times larger than the true marginal effect of 0.1, because stress-driven variation was incorrectly attributed to social media use.

Fourth, subset analysis and graphical diagnostics provide valuable tools for identifying when regression assumptions are violated. By restricting the analysis to observations where the proxy maintains a more linear relationship with the true variable, we were able to recover the correct Time coefficient and identify violations of the linearity assumption through residual diagnostics.

Finally, the comparison between models using proxy versus true measurements illustrates how measurement choices fundamentally shape both scientific inference and public interpretation of research findings. Models with identical statistical credentials can produce opposite substantive conclusions depending solely on the quality of variable measurement.

These findings underscore the importance of carefully evaluating measurement quality, examining model assumptions through graphical diagnostics, and interpreting regression coefficients in the context of theoretical expectations rather than relying solely on summary statistics and significance tests. When proxy variables must be used, researchers should assess the functional form of the relationship between proxies and true variables, consider subset analyses to identify regimes where linearity holds, and remain cautious about substantive interpretations when proxies exhibit non-linear scaling patterns.
